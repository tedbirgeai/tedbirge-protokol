...
const TITLE = "Canlı Mesh Demosu — Tedbirge Gateway Simülasyonu";
const DESC =
...
const URL = "https://artisan-project-studio.lovable.app/demo";
...
function Demo() {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      last = now;
      setProgress((p) => p + dt * (path ? 0.55 : 0));
        ...
        <div className="rounded-sm border border-border bg-card/40 p-6">
          <div className="relative aspect-[16/10] w-full">
            <svg viewBox="0 0 100 100" className="h-full w-full" preserveAspectRatio="none">
              <Link2 from="A" to="B" active={relayUp} />
              <Link2 from="B" to="C" active={relayUp} />
              <Link2 from="A" to="C" active={!relayUp && directLink} dashed />
              {NODES.map((n) => (
                <g key={n.id}>
                  <circle
                    cx={n.x}
              ...
              {packet && (
                <circle cx={packet.x} cy={packet.y} r={1.8} className="fill-accent" />
              )}
            </svg>
            {NODES.map((n) => (
              <div
